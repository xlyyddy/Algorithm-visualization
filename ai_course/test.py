...
def update(stack):
    ...
    nx.draw_networkx_edge_labels(G, pos, 
                                ...
                                label_pos=0.5,  # 标签位置优化
                                font_size=15,  # 增大字体
                                font_weight='bold',  # 加粗
                                bbox=dict(alpha=0))
    nx.draw_networkx_edges(G, pos, edgelist=edges, width=2)  # 根据权值设置边的粗细
    plt.text(12.5, 20, f"{m}", ha='center', va='bottom', fontsize=50, fontweight='bold', color='black')
    plt.axis('off')
